...
graph TD
    ...
    planner -->|"Send fan-out ×N (una rama por sub-pregunta)"| search_worker["search_worker ×N<br/>paralelo"]
    search_worker --> aggregate_search[aggregate_search]
    aggregate_search -->|"Send fan-out ×N (una rama por URL)"| scrape_worker["scrape_worker ×N<br/>paralelo"]
    scrape_worker --> aggregate_documents[aggregate_documents]
    aggregate_documents -->|"Send fan-out ×N (una rama por doc)"| document_worker["document_worker ×N<br/>paralelo"]
